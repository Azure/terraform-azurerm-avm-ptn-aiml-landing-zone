locals {
  ...
  subnets_definition    = var.byo_vnet_definition != null ? var.byo_vnet_definition.subnets : var.vnet_definition.subnets
  virtual_network_links = merge(local.default_virtual_network_link, var.private_dns_zones.network_links)
  vnet_address_space    = var.byo_vnet_definition != null ? data.azurerm_virtual_network.ai_lz_vnet[0].address_space[0] : var.vnet_definition.address_space
  vnet_name             = var.byo_vnet_definition != null ? basename(var.byo_vnet_definition.resource_id) : (try(var.vnet_definition.name, null) != null ? var.vnet_definition.name : (var.name_prefix != null ? "${var.name_prefix}-vnet" : "ai-alz-vnet"))
