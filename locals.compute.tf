locals {
  ...
  container_app_environment_name                     = try(var.container_app_environment_definition.name, null) != null ? var.container_app_environment_definition.name : (try(var.name_prefix, null) != null ? "${var.name_prefix}-container-app-env" : "ai-alz-container-app-env-${random_string.name_suffix.result}")
