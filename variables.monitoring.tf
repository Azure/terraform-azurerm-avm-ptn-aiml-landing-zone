...
- `deploy` - (Optional) Boolean to indicate whether to deploy a new Log Analytics Workspace if no resource_id is provided. Default is true. Set to false together with no resource_id to disable automatic diagnostic settings management for all resources (useful when policy-driven diagnostic settings are in place).
