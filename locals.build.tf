locals {
  ...
  build_key_vault_name = try(var.build_key_vault_definition.name, null) != null ? var.build_key_vault_definition.name : (var.name_prefix != null ? "${var.name_prefix}-build-kv-${random_string.name_suffix.result}" : "build-kv-${random_string.name_suffix.result}")
